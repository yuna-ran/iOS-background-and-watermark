...
class BackgroundAndWatermarkEntry : UIViewController
{
    @IBOutlet weak var joinButton: UIButton!
    @IBOutlet weak var channelTextField: UITextField!
    
    let identifier = "BackgroundAndWatermark"
...
class BackgroundAndWatermarkMain: BaseViewController {
    ...
    @IBAction func backgroundSwitched(_ sender: UISwitch) {
        isVirtualBackgroundEnabled = sender.isOn
        LogUtils.log(message: "backgroundSwitched", level: .info)
        setBackGround()
    }
    
    
    @IBAction func watermarkSwitched(_ sender: UISwitch) {
      //  LogUtils.log(message: "watermarkSwitched", level: .info)
        if sender.isOn {
            addWatermark()
        } else {
    ...
    private func setBackGround() {
        LogUtils.log(message: "currently add background is" + String(isVirtualBackgroundEnabled), level: .info)
     
        let backgroundSource = AgoraVirtualBackgroundSource()
        backgroundSource.backgroundSourceType = .img
        ...
        LogUtils.log(message: "resource path is " + String(backgroundSource.source ?? "default value"), level: .info)
...
extension BackgroundAndWatermarkMain: AgoraRtcEngineDelegate {
